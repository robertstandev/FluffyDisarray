...
public class ColliderUpdater : MonoBehaviour
{
    ...
    public void updateCollider(SpriteRenderer spriteRenderer, PolygonCollider2D polygonCollider)
    {
        checkSpriteChange(spriteRenderer);
        if (this.hasSpriteChanged)
        {
            recalculateSpritePolygons(spriteRenderer, polygonCollider);
        }
    }

    private void checkSpriteChange(SpriteRenderer spriteRenderer)
    {
        if (spriteRenderer.sprite != this.lastSprite)
        {
            this.lastSprite = spriteRenderer.sprite;
            this.hasSpriteChanged = true;
        }
    ...
    private void recalculateSpritePolygons(SpriteRenderer spriteRenderer, PolygonCollider2D polygonCollider)
    {
        if (spriteRenderer.sprite != null)
        {
            ...
            updatePolygonCollider(spriteRenderer, polygonCollider);
        }
    }

    private void updatePolygonCollider(SpriteRenderer spriteRenderer, PolygonCollider2D polygonCollider)
    {
        ...
        this.hasSpriteChanged = false;
        populateCollider(spriteRenderer, polygonCollider);
    }

    private void populateCollider(SpriteRenderer spriteRenderer, PolygonCollider2D collider)
    {
        try
        {
            ...
            {
                processVertices(spriteRenderer, collider, verts[i], list, ref pathIndex);
            }
        }
        catch (Exception exception)
        {
            Debug.LogError("Error creating collider: " + exception);
        }
    }

    private List<Vector2[]> processVertices(SpriteRenderer spriteRenderer, PolygonCollider2D collider, Vertices v, List<Vector2[]> list, ref int pathIndex)
    {
...
sealed class TextureConverter
{
    ...
    private bool InBounds(ref Vector2 coord)
    {
